...
CREATE POLICY "Allow users to update open_games" ON public.open_games FOR UPDATE USING (auth.uid()::text = host_user_id OR host_user_id = 'mock_user' OR auth.role() = 'authenticated');
