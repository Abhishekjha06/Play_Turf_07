...
import { Upload, CheckCircle2, Loader2, MessageSquarePlus } from 'lucide-react';
...
export const FeedbackModal: React.FC<FeedbackModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="card-panel relative flex flex-col gap-4 overflow-hidden rounded-3xl p-6 shadow-2xl bg-panel-2 border-0 max-w-lg max-h-[90dvh]">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-primary/20 text-primary">
            <MessageSquarePlus className="h-5 w-5" />
          </div>
          <div>
            <DialogTitle className="text-lg font-bold font-display">Send Feedback</DialogTitle>
            <DialogDescription className="text-xs text-muted2">Help us improve your experience</DialogDescription>
          </div>
        </div>
